...
### 发布模型部署
#### 基于DAG引擎创建算子流水线并发布（使用eagleeye扩展包）
```
# 创建管线并编译
# deploy.xxx 是C++算子（支持动态编译），支持python端调用和c++端调用
# eagleeye.xxx 是eagleeye核心算子，支持python端调用和c++端调用
# xxx_op 算子是python端和c++端独立编写，依靠人工审核实现执行一致
# inference_onnx_op 模型推断算子，在发布时，将自动转换成设置的推断引擎，如下代码是使用snpe引擎进行模型预测

# build(...)， platform参数设置
# SDK 发布：
# android/arm64-v8a, linux/x86-64
# 服务发布：
# cloud/microservice, private/service
# project_config 项目信息配置
# input 指定模块输入信息
# output 指定模块输出信息
# name 指定模块名字
# git 指定模块的git地址，指定后将在直接拉去此代码仓库

placeholder['image'](np.ones((384,512,3), dtype=np.uint8)). \
    resize_op['image', 'resized_image'](out_size=(512,384)). \
    deploy.preprocess_func['resized_image', 'preprocessed_image'](mean=np.array([0.5,0.5,0.5], dtype=np.float32), std=np.array([1,1,1], dtype=np.float32), rgb2bgr=True). \
    inference_onnx_op[
        'preprocessed_image', ('heatmap_level_1', 'heatmap_level_2', 'heatmap_level_3', 'offset_level_1', 'offset_level_2', 'offset_level_3')
    ](
        ...
        engine_args={
            ...
            'model': '/workspace/bb-epoch_16-model.dlc',
        }). \
    build(
