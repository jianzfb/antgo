# -*- coding: UTF-8 -*-
# @Time    : 2018/11/22 2:46 PM
# @File    : layer_transformer.py
...
class LayerTransfomer:
  def __init__(self, layer_factory):
    self.layer_factory = layer_factory

  def deeper_conv_block(self, conv_layer, kernel_size, weighted=True):
    n_dim = conv_layer.n_dim
    n_filters = conv_layer.filters

    new_conv_layer = getattr(self.layer_factory, 'conv%dd'%n_dim)(conv_layer.filters, n_filters, kernel_size=kernel_size)
    bn = getattr(self.layer_factory, 'bn%dd'%n_dim)(n_filters)
    ...
    return [self.layer_factory.relu(),
            new_conv_layer,
            bn]

  def wider_next_conv(self, layer, start_dim, total_dim, n_add, weighted=True):
    n_dim = layer.n_dim
    ...
    new_conv_layer = getattr(self.layer_factory, 'conv%dd'%n_dim)(layer.input_channel + n_add,
                                                                 layer.filters,
                                                                 kernel_size=layer.kernel_size)
    return new_conv_layer

  def wider_next_dense(self, layer, start_dim, total_dim, n_add, weighted=True):
    # teacher_w, teacher_b = layer.get_weights()
    ...
    # new_layer.set_weights((student_w, teacher_b))
    # return new_layer
    return self.layer_factory.dense(layer.input_units + n_add, layer.units)

  def wider_bn(self, layer, start_dim, total_dim, n_add, weighted=True):
    ...
    n_dim = layer.n_dim
    new_bn_layer = getattr(self.layer_factory, 'bn%dd'%n_dim)(layer.num_features + n_add)
    return new_bn_layer

  def wider_pre_conv(self, layer, n_add_filters, weighted=True):
    ...
    n_dim = layer.n_dim
    new_conv_layer = getattr(self.layer_factory, 'conv%dd'%n_dim)(layer.input_channel,
                                                                 layer.filters + n_add_filters,
                                                                 kernel_size=layer.kernel_size)
    return new_conv_layer

  def wider_pre_dense(self, layer, n_add, weighted=True):
    ...
    return self.layer_factory.dense(layer.input_units, layer.units + n_add)

  def dense_to_deeper_block(self, dense_layer, weighted=True):
    units = dense_layer.units
    new_dense_layer = self.layer_factory.dense(units, units)
    # if weighted:
    #   weight = np.eye(units)
    #   bias = np.zeros(units)
    #   new_dense_layer.set_weights((add_noise(weight, np.array([0, 1])), add_noise(bias, np.array([0, 1]))))

    return [self.layer_factory.relu, new_dense_layer]
