...
class For(object):
    def __init__(self, func, parallel_num=1, is_warp_to_numpy=False, **kwargs):
        self.func = func
    ...
    def __call__(self, *args, **wargs):
        out_list = None
        loop_num = max([len(v) for v in args if not isinstance(v, str)])
        if min([len(v) for v in args if not isinstance(v, str)]) == 0:
            return np.empty((0), dtype=np.float32)

        for loop_i in range(loop_num):
            data_tuple = [v if isinstance(v, str) else v[loop_i%len(v)] for v in args]
