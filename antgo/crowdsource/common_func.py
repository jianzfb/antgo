...
def select_recv(conn, buff_size, timeout=None):
  ...
  """
  poller = select.poll()
  poller.register(conn, select.POLLIN | select.POLLPRI | select.POLLHUP | select.POLLERR)
  events = poller.poll(timeout)
  buff = None
  for fd, flag in events:  # iter every read-ready or closed sockets
    assert(conn.fileno() == fd)
...
class SocketBridge:
  ...
  def __init__(self):
    ...
    self.fd_to_sokcet={}    # map file descriptors to socket objects
  ...
  def add_conn_pair(self, conn1, conn2, callback=None):
    ...
    self.poller.register(conn1, select.POLLIN | select.POLLPRI | select.POLLHUP | select.POLLERR)
    self.poller.register(conn2, select.POLLIN | select.POLLPRI | select.POLLHUP | select.POLLERR)
    self.fd_to_sokcet[conn1.fileno()] = conn1
    self.fd_to_sokcet[conn2.fileno()] = conn2
  ...
  def _start(self):
    ...
    while True:
      ...
      # waiting until some socket have data
      events = self.poller.poll(1000)

      for fd, flag in events:  # iter every read-ready or closed sockets
        s = self.fd_to_sokcet[fd]
        if flag & (select.POLLIN | select.POLLPRI):
          # readable event
          try:
          ...
          # close s and its pair
          try:
            self._rd_shutdown(s)
            self._rd_shutdown(self.map[s])
          except:
            continue
  ...
  def _rd_shutdown(self, conn, once=False):
    ...
    if conn.fileno() in self.fd_to_sokcet:
      self.fd_to_sokcet.pop(conn.fileno())

    #
    log.info('keeping the remaining %d sockets'%len(self.fd_to_sokcet))
