...
class BaseDenseHead(BaseModule, metaclass=ABCMeta):
    ...
    def _bbox_post_process(self,
        ...
        if with_nms and cfg.nms is not None:
            if mlvl_bboxes.numel() == 0:
                det_bboxes = torch.cat([mlvl_bboxes, mlvl_scores[:, None]], -1)
                return det_bboxes, mlvl_labels

            keep_idxs = batched_nms(mlvl_bboxes, mlvl_scores,
                                                mlvl_labels, cfg.nms['iou_threshold'])

            det_bboxes = mlvl_bboxes[keep_idxs][:cfg.max_per_img]
            det_scores = mlvl_scores[keep_idxs][:cfg.max_per_img]
            det_bboxes = torch.cat([det_bboxes, det_scores.unsqueeze_(-1)], dim=-1)
    ...
    def simple_test(self, feats, image_metas, rescale=True, **kwargs):
        ...
        Returns:
            list[tuple[Tensor, Tensor]]: Each item in result_list is 2-tuple.
                ...
                with shape (n, ).
        """
        return self.simple_test_bboxes(feats, image_metas, rescale=rescale, **kwargs)
