...
from antgo.dataflow.structures.bbox.transforms import get_udp_warp_matrix, get_warp_matrix
...
class TopdownAffine(BaseTransform):
    ...
    def transform(self, results: Dict) -> Optional[dict]:
        ...
        if isinstance(results['image'], list):
            results['image'] = [
                cv2.warpAffine(
            ...
            results['image'] = cv2.warpAffine(
                results['image'], warp_mat, warp_size, flags=cv2.INTER_LINEAR)
        ...
        if results.get(keypoints_key, None) is not None:
            transformed_keypoints = results[keypoints_key].copy()
            # Only transform (x, y) coordinates
            transformed_keypoints[..., :2] = cv2.transform(
                results[keypoints_key][..., :2], warp_mat)
            results[keypoints_key] = transformed_keypoints
