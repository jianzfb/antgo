...
from antgo.framework.helper.dataset.pipelines.utils import cache_randomness
from scipy.stats import truncnorm
from antgo.framework.helper.dataset.builder import PIPELINES
from antgo.dataflow.structures.bbox.transforms import (bbox_clip_border, bbox_corner2xyxy,
                                    bbox_xyxy2corner, get_pers_warp_matrix,
                                    get_udp_warp_matrix, get_warp_matrix)
from antgo.dataflow.structures.keypoint.transforms import keypoint_clip_border
...
class BottomupRandomAffine(BaseTransform):
    ...
    def transform(self, results: Dict) -> Optional[dict]:
        ...
        results['image'] = self._transform(results['image'], warp_mat,
                                         (int(w), int(h)))

        keypoints_key = 'keypoints' if 'keypoints' in results else 'joints2d'
        keypoints_visible_key = 'keypoints_visible' if 'keypoints_visible' in results else 'joints_vis'
        if keypoints_key in results:
            # Only transform (x, y) coordinates
            kpts = cv2.transform(results[keypoints_key], warp_mat)
            if kpts.shape[-1] == 3:
                kpts = kpts[..., :2] / kpts[..., 2:3]
            results[keypoints_key] = kpts

            if self.clip_border:
                results[keypoints_key], results[keypoints_visible_key] = \
        ...
        if 'bboxes' in results:
            bbox = bbox_xyxy2corner(results['bboxes'])
        ...
        # for person_points,person_points_vis in zip(results[keypoints_key], results[keypoints_visible_key]):
...
class BottomupResize(BaseTransform):
    ...
    def transform(self, results: Dict) -> Optional[dict]:
        ...
        actual_input_size, padded_input_size = self._get_input_size(
            img_size=(img_w, img_h), input_size=(w, h))
        ...
        if keypionts_key in results:
            # Only transform (x, y) coordinates
            kpts = cv2.transform(results[keypionts_key], warp_mat)
            if kpts.shape[-1] == 3:
                kpts = kpts[..., :2] / kpts[..., 2:3]

            homogeneous_warp_mat = np.concatenate([warp_mat, np.array([[0,0,1]], dtype=np.float32)], 0)
            inv_homogeneous_warp_mat = np.linalg.inv(homogeneous_warp_mat)            
            results[keypionts_key] = kpts

            if self.clip_border:
                results[keypionts_key], results[
                    keypoints_visible_key] = keypoint_clip_border(
                        results[keypionts_key], results[keypoints_visible_key],
                        (w, h))
        ...
        # 正向变换
        results['warp_mat'] = warp_mat.astype(np.float32)
        # 反向变换
        homogeneous_warp_mat = np.concatenate([warp_mat, np.array([[0,0,1]], dtype=np.float32)], 0)
        inv_homogeneous_warp_mat = np.linalg.inv(homogeneous_warp_mat)
        results['inv_warp_mat'] = inv_homogeneous_warp_mat[:2,:3].astype(np.float32)
        return results
...
class BottomupRandomCrop(BaseTransform):
    ...
    def _crop_data(self, results: dict, crop_size: Tuple[int, int],
        ...
        Returns:
            results (Union[dict, None]): Randomly cropped results, 'image_shape'
        ...
        img_shape = img.shape
        results['image'] = img
        results['image_shape'] = img_shape[:2]

        # crop bboxes accordingly and clip to the image boundary
        if results.get('bboxes', None) is not None:
            ...
            results['bboxes'] = bboxes[valid_inds]
            meta_keys = [
                'bboxes_score', 'id', 'category_id', 'labels', 'raw_ann_info', 'iscrowd'
            ]
            ...
            if results.get(keypionts_key, None) is not None:
                ...
                if self.bbox_clip_border:
                    ...
                    results[keypoints_visible_key][kpt_outside] *= 0
                keypoints[:, :, 0] = keypoints[:, :, 0].clip(0, img_shape[1])
                keypoints[:, :, 1] = keypoints[:, :, 1].clip(0, img_shape[0])
                results[keypionts_key] = keypoints[valid_inds]
                results[keypoints_visible_key] = results[keypoints_visible_key][
                    valid_inds]
            ...
            if results.get('masks', None) is not None:
                ...
                if self.recompute_bbox:
                    results['bboxes'] = results['masks'].get_bboxes(type(results['bboxes']))
    ...
    def transform(self, results: dict) -> Union[dict, None]:
        ...
        Returns:
            results (Union[dict, None]): Randomly cropped results, 'image_shape'
                ...
                be returned when there is no valid bbox after cropping.
        """
        image_size = results['image'].shape[:2]
...
class BottomupRandomChoiceResize(BaseTransform):
    ...
    def _resize_img(self, results: dict) -> None:
        ...
        results['image'] = img
        results['image_shape'] = img.shape[:2]
        ...
        if results.get('bbox', None) is not None:
            ...
            if self.clip_object_border:
                bboxes[:, 0::2] = np.clip(bboxes[:, 0::2], 0,
                                          results['image_shape'][1])
                bboxes[:, 1::2] = np.clip(bboxes[:, 1::2], 0,
                                          results['image_shape'][0])
            results['bbox'] = bboxes
        ...
        if results.get(keypionts_key, None) is not None:
            ...
            if self.clip_object_border:
                ...
                keypoints[:, :, 1] = np.clip(keypoints[:, :, 1], 0,
                                             results['image_shape'][0])
            results[keypionts_key] = keypoints
    ...
    def transform(self, results: dict) -> dict:
        ...
        Returns:
            dict: Resized results, 'image', 'bbox',
            'keypoints', 'scale', 'scale_factor', 'image_shape',
