...
from antgo.dataflow.imgaug.operators_3d import *
...
class InterHand26M(Dataset):
    def __init__(self, train_or_test, dir=None, params=None):
        super(InterHand26M, self).__init__(train_or_test, dir, params)
    
        self.mode = train_or_test # train, test, val
        self.img_path = os.path.join(dir, 'images')
        self.annot_path = os.path.join(dir, 'annotations')
        if self.mode == 'val':
            self.rootnet_output_path = os.path.join(dir,'rootnet_output/rootnet_interhand2.6m_output_val.json')
        else:
            self.rootnet_output_path = os.path.join(dir,'rootnet_output/rootnet_interhand2.6m_output_test.json' )
        ...
        trans_test = getattr(self, 'trans_test', '')
        if (self.mode == 'val' or self.mode == 'test') and trans_test == 'rootnet':
        ...
        for aid in db.anns.keys():
            ...
            if (self.mode == 'val' or self.mode == 'test') and trans_test == 'rootnet':
                ...
                input_img_shape = getattr(self, 'input_img_shape', None)
    ...
    def at(self, idx):
        ...
        # augmentation
        input_img_shape = getattr(self, 'input_img_shape', None)
        output_hm_shape = getattr(self, 'output_hm_shape', '')
        bbox_3d_size = getattr(self, 'bbox_3d_size', 0)
        output_root_hm_shape = getattr(self, 'output_root_hm_shape', 0)
        bbox_3d_size_root = getattr(self, 'bbox_3d_size_root', 0)
        img, joint_coord, joint_valid, hand_type, inv_trans = augmentation(img, bbox, joint_coord, joint_valid, hand_type, self.mode, self.joint_type, input_img_shape)
        ...
        joint_coord, joint_valid, rel_root_depth, root_valid = transform_input_to_output_space(joint_coord, joint_valid, rel_root_depth, root_valid, self.root_joint_idx, self.joint_type, input_img_shape, output_hm_shape, bbox_3d_size, output_root_hm_shape, bbox_3d_size_root)
    ...
    def _set_group_flag(self):
        ...
        otherwise group 0.
        """
        self.flag = np.zeros(self.size, dtype=np.uint8)
    ...
    def evaluate(self, preds, **kwargs):
        ...
        for n in range(sample_num):
            ...
            pred_joint_coord_img = pred_data['joint_coord'].copy()
            output_hm_shape = getattr(self, 'output_hm_shape', [])
            input_img_shape = getattr(self, 'input_img_shape', [])
            output_hm_shape = [(int)(a) for a in output_hm_shape]
            input_img_shape = [(int)(a) for a in input_img_shape]
            pred_joint_coord_img[:,0] = pred_joint_coord_img[:,0]/output_hm_shape[2]*input_img_shape[1]
            pred_joint_coord_img[:,1] = pred_joint_coord_img[:,1]/output_hm_shape[1]*input_img_shape[0]
            for j in range(self.joint_num*2):
                pred_joint_coord_img[j,:2] = trans_point2d(pred_joint_coord_img[j,:2],pred_data['inv_trans'])
            # restore depth to original camera space
            bbox_3d_size = getattr(self, 'bbox_3d_size', 0)
            pred_joint_coord_img[:,2] = (pred_joint_coord_img[:,2]/output_hm_shape[0] * 2 - 1) * (bbox_3d_size/2)

            # mrrpe
            output_root_hm_shape = getattr(self, 'output_root_hm_shape', 0)
            bbox_3d_size_root = getattr(self, 'bbox_3d_size_root', 0)
            if gt_hand_type == 'interacting' and joint_valid[self.root_joint_idx['left']] and joint_valid[self.root_joint_idx['right']]:
                pred_rel_root_depth = (pred_data['rel_root_depth']/output_root_hm_shape * 2 - 1) * (bbox_3d_size_root/2)
            ...
            # handedness accuray
            if hand_type_valid:
                if gt_hand_type == 'right' and pred_data['hand_type'][0] > 0.5 and pred_data['hand_type'][1] < 0.5:
                    acc_hand_cls += 1
                elif gt_hand_type == 'left' and pred_data['hand_type'][0] < 0.5 and pred_data['hand_type'][1] > 0.5:
                    acc_hand_cls += 1
                elif gt_hand_type == 'interacting' and pred_data['hand_type'][0] > 0.5 and pred_data['hand_type'][1] > 0.5:
                    acc_hand_cls += 1
                hand_cls_cnt += 1
