...
class BaseWork(AntBase):
  class _LinkPipe(object):
    ...
    def put(self, sender='', identity='', value=''):
      self._queue.put(json.dumps({'sender': sender,
                                  'identity': identity,
                                  'value': value}))
  ...
  def __init__(self, name, code_path, code_main_file, config_parameters, port=None, ant_name=None, ant_token=None):
    super(BaseWork, self).__init__(ant_name=ant_name, ant_token=ant_token)
  ...
  def save_config(self, config_parameters):
  ...
  def trigger(self, value, identity='CONTINUE'):
    if identity == 'DONE' and \
            len(self._feedback_input) > 0 and not self._need_waiting_feedback:
  ...
  def _prepare_running_data(self, identity_list, info_list):
    ...
    for identity, info in zip(identity_list, info_list):
      if self._is_root:
        if len(self.feedback_input) > 0 and \
              (self.status == 'FEEDBACK-DONE' or self.status == 'STOP') and \
              identity == 'STOP':
          self.trigger('', identity='STOP')
          return 0
      ...
      for record in info:
        ...
        if os.path.isfile(record):
          ...
          for ff in os.listdir(record):
            ...
            record_folder = ''
            if len(info) > 1:
              record_folder = record.split('/')[-1]
              if not os.path.exists(os.path.join(self.dump_dir, record_folder)):
                os.makedirs(os.path.join(self.dump_dir, record_folder))

            if os.path.isdir(os.path.join(record, ff)):
              shutil.copytree(os.path.join(record, ff), os.path.join(self.dump_dir, record_folder, ff))
            elif os.path.isfile(os.path.join(record, ff)):
              shutil.copyfile(os.path.join(record, ff), os.path.join(self.dump_dir, record_folder, ff))
  ...
  def waiting_and_run(self, acquired_lock=None):
    while True:
      if len(self._feedback_input) > 0:
        if self.status == 'FEEDBACK-DONE' or \
                self.status == 'STOP' or \
                not self.need_waiting_feedback or \
                self._is_first:
