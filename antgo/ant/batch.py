...
class AntBatch(AntBase):
    ...
    def start(self):
        ...
        if self.from_experiment is not None:
            experiment_uuid = self.from_experiment
        ...
        # 启动bach server
        self.launch_web(experiment_dump_dir)

        # 是否来自已有实验
        if self.from_experiment is not None:
            # 加载保存的执行信息
            running_info = {}
            with open(os.path.join(experiment_dump_dir, '%s.json' % self.from_experiment), 'r') as fp:
        ...
        if len(running_ant_task.dataset_name.split('/')) == 2:
            ...
            selected_dataset_stages = ['val', 'test']
