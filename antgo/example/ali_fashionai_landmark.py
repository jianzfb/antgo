...
class HourglassModel(ModelDesc):
  ...
  def model_input(self, is_training, data_source):
    ...
    qq = DatasetQueue(reorganized_data,
                      ...
                      [(ctx.params.input_size, ctx.params.input_size, 4),
                       ...
                       (24,)])
    image, gtmap, pick_index = qq.dequeue()
    ...
    # make batch
    rotated_images, rotated_gtmaps, pick_indexs = tf.train.batch([rotated_image, rotated_gtmap, pick_index], ctx.params.batch_size)
    data_queue = slim.prefetch_queue.prefetch_queue([rotated_images, rotated_gtmaps, pick_indexs])
    return data_queue
  ...
  def model_fn(self, is_training=True, *args, **kwargs):
    ...
    if is_training:
      inputs, labels, pick_indexs = args[0].dequeue()
    else:
      ...
      with tf.variable_scope('model'):
        ...
        with tf.variable_scope('stacks'):
          ...
          if is_training:
            ...
            # more or less
            positive_weight = labels * 50.0
            
            sample_weight = hard_weight * positive_weight
            ...
            # # attention on category joints (local attention)
            # loss_mat_reshpae = tf.transpose(loss_mat, perm=(0, 4, 1, 2, 3))
            # loss_mat_reshpae = tf.reshape(loss_mat_reshpae,
            #   (-1, ctx.params.nstacks, ctx.params.hm_size, ctx.params.hm_size))
            #
            # useful_index = tf.reshape(test_pick, (-1,))
            # useful_index = tf.where(tf.equal(useful_index, 1))
            # useful_index = tf.squeeze(useful_index, 1)
            # loss_mat_reshpae = tf.gather(loss_mat_reshpae, useful_index)
            #
            # #
            # loss_v = tf.reduce_mean(loss_mat_reshpae)
...
def _data_reorganize(*args, **kwargs):
  ...
  print(train_img_annotation['category'])
  print(index_pick)
  ...
  hm = _generate_hm(ctx.params.hm_size, ctx.params.hm_size, joints, ctx.params.hm_size/1.5, train_weight)
  hm = np.expand_dims(hm, axis=0)
  hm = np.repeat(hm, ctx.params.nstacks, axis=0)
  
  return train_img, hm, index_pick
