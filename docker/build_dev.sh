...
# 1. 交互式启动容器
# sudo docker run -it --rm --name myenv --gpus all antgo-env /bin/bash
# 2. vscode-server服务
# sudo docker run --rm -d -p 8080:8080 -e PASSWORD=123 --name mycodeserver --gpus all antgo-env-dev /opt/code-server --host 0.0.0.0 --auth password